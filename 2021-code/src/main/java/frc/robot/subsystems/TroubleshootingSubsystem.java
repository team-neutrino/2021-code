...
public class TroubleshootingSubsystem extends SubsystemBase
{
    ...
    public TroubleshootingSubsystem(ShooterSubsystem p_Shooter, DriveSubsystem p_Drive, IntakePIDSubsystem p_Intake) {
        ...
        m_shooter_velocity_two = m_troubleshooting_tab.add("Shooter Velocity Two", 0).withPosition(1, 0).withSize(2, 2).withWidget(
            BuiltInWidgets.kDial).withProperties(Map.of("min", 0, "max", 120000)).getEntry();
        m_input_shooter_Speed = m_troubleshooting_tab.add("Input Shooter Velocity", 0).withPosition(1,1).withSize(1,1).getEntry();

        m_navx_yaw = m_troubleshooting_tab.add("NavX Yaw", 0).withPosition(2, 3).withSize(1, 1).withProperties(Map.of("min", -180, "max", 180)).getEntry();
